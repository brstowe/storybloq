...
- **The PEN = the session, and it is spent sparingly.** Wave planning, acceptance, the rulings only it should make, and ledger writes stay at the session tier because they ARE the session. Everything it can hand down, it hands down: on an expensive session tier, labor typed by the pen is the single most wasteful thing the loop can do. A ruling the pen makes is RECORDED, not repeated: `ruling create --cites` binds it to the items it governs in one write, and every downstream tier then reads it from the item instead of from whatever the dispatch prompt happened to quote.
...
**Pass the pin through to the record.** The tier you chose is only half-recorded if it lives in your dispatch and nowhere else. When the dispatched agent reports a review round or `implementation_done`, it passes `reviewerModel`/`reviewerTier` or `implementerModel`/`implementerTier`, so the ledger can afterwards answer which tier reviewed and which tier implemented an item rather than leaving it to be reconstructed from memory. A pin recorded this way is stored as `evidence: "configured"` -- it is what you ASKED FOR, never proof of what ran, and only a backend reporting its own execution may be sent as `observed`. Pass nothing rather than a guess: `unknown` is a truthful record and a plausible model name is not.
...
- **Sizing sets `reviewEffort`, and tier and effort are separate axes.** The tier decides WHO does the work; the effort dial decides how hard review works, and a wave FILES both per item: trivial/mechanical -> `light`, XS/S/M -> `standard`, L or risk-flagged -> `thorough`. This is a filing policy, not the automatic mapping: left unset, autonomous mode selects `light` only for a low-risk `chore` (there is no size field to read), so a wave that wants a trivial item reviewed lightly has to say so in metadata. `off` is never a wave default: an item nobody reviews is an item nobody can ship on evidence.
...
XS/chore items may collapse stages 1-2 into the implementation prompt, DISCLOSED (the report states the 5-line plan). No stage is ever skipped silently. `reviewEffort: light` is the autonomous analogue in SPIRIT, not in mechanism: it lowers the depth of both review stages and discloses it, but it never skips either gate. Only `reviewEffort: off` removes a stage there.
