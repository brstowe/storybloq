...
| wake (idle) | immediately after a send, when the peer is idle | the SENDING process makes one client-native wake attempt: for a Codex endpoint, one `turn/start` on the app-server daemon's control socket; for a Claude endpoint, the sending Claude session follows the send with `SendMessage`. Opt-in per endpoint with `bus setup --wake idle`; wakePolicy defaults to `never` | Codex threads a local daemon already owns; Claude peers visible in the sender's `ListAgents` |
...
The wake tier starts a turn on an idle peer so a message does not sit in the mailbox until the owner types "check the Bus". It is best-effort and never a delivery path: the mail is committed to the mailbox first, the wake is attempted once, and delivery still happens through a poll. Nothing about a wake is retried, coalesced, or rate-limited in this cut; one send makes at most one attempt. The attempt is recorded in two places that do not hold the same thing: the ENDPOINT keeps the last outcome of every kind (`lastWakeAt` and `lastWakeResult`, skips included), while a durable thread `wake` entry is appended for `requested`, `failed:<reason>` and `poll_observed`. Skips never reach the thread, and that is deliberate rather than an omission: a skip is a local decision that never touched the peer, and the thread is shared, hash-chained and visible to the other participant, so one side's gating has no business being published there. The endpoint's last outcome is the only place a skip is visible, and `storybloq bus endpoint list` is where to read one endpoint's.

A successful `turn/start` only proves the server accepted a turn. The evidence that mail actually reached someone is `poll_observed`, which the poll path appends when the woken endpoint's polled mailbox cursor stands at or past the cursor recorded at wake time. That condition is cumulative: it is about the position the endpoint has reached, not about whether one particular poll crossed it, so a poll that surfaces nothing new can still record the observation. It is an observation and never a cause, because a peer polls when its owner types "check the Bus" as readily as when a turn was started for it. Read the counts with `storybloq bus status` (add `--format json` for the structured shape). The skip tally in the same section counts ENDPOINTS by their most recent outcome and is not a population, because `lastWakeResult` holds one overwritten value per endpoint.

What that section reports is a lower bound rather than a measurement. Recording one observation needs a single poll to do three things: DISCOVER the `requested` entry on a thread it folded, hold a cursor at or past that entry's wake cursor, and append the observation successfully. When any of the three does not happen, the recording waits on a later poll folding that thread again, which is what happens as soon as fresh mail lands on it; where no later poll ever does, the wake stays `requested` even though its mail was read. Three ways that plays out: an ack reclaims the thread's mailbox pointer before any poll reaches the wake cursor, so nothing folds it again until new mail arrives; the `requested` entry is written after the poll that would have carried it had already folded that thread; or the observation's own append fails. None of them means the mail went unread, which is exactly why the number is reported as a floor.

Opting in. `wakePolicy` has existed since the first Bus cut and every endpoint joins with `never`, so endpoints created before 1.14.0 defaulted to `never` and remain opted out unless someone explicitly changed them. They are opted out, not broken: nothing needs repairing, and the `--session-name` and `--transport-address` flags are not involved in whether a Codex wake fires. Run `storybloq bus endpoint list` to see each endpoint's current policy and last wake outcome, then `storybloq bus setup --wake idle` to opt one in. That flag is accepted only for a Codex CLI endpoint: setup refuses it for a Claude client, which is reached by the native `SendMessage` path instead, and for the `codex_desktop` surface, whose app-server no external process can connect to.

Codex side. The wake connects to the app-server daemon's control socket (`codex app-server daemon start` creates it; the socket exists only while a daemon runs, so its absence is a normal state), reads the thread's status from `thread/list`, confirms this daemon owns the thread from `thread/loaded/list`, and issues `turn/start` with exactly `{threadId, input}`: plain text telling the peer to run `storybloq bus poll`. It never passes an approval policy, sandbox policy, or any other setting, never calls `turn/steer`, and never resumes a thread it does not own. Every other case is recorded rather than attempted: a thread this daemon does not own is `skipped:ownership-unproven`; a Codex Desktop task, whose app-server is private to the desktop app, is `skipped:surface-unreachable`; an active turn is `skipped:active-turn`, and the Stop hook path stays the only delivery during an active turn, with the two paths deduplicated on the mailbox cursor so a message is never delivered twice; a daemon of an unpinned version is `failed:version` (the observed version is daemon-supplied text and is never copied into the record). The full set of recorded outcomes is `skipped:not-codex`, `skipped:already-polled-through-batch`, `skipped:pending-unknown`, `skipped:thread-not-found`, `skipped:status-unknown`, `skipped:thread-system-error`, `skipped:ownership-unproven`, `skipped:surface-unreachable`, `skipped:active-turn`, `failed:version`, `failed:lookup-incomplete`, `failed:socket-absent`, `failed:handshake`, `failed:frame`, `failed:closed`, `failed:timeout`, and `failed:io`.

Claude side. A Claude session that sends Bus mail follows the send with a `SendMessage` to the peer's session when `ListAgents` shows it. The address it uses is the endpoint's `clientSessionName`, which is captured at setup from `ListAgents` and nowhere else: no session knows its own address, the harness assigns it, and both ends only ever learn it from that listing. When the sender is Codex, a Claude peer is reached by its own armed rendezvous instead, so arming `bus poll --wait` at every idle boundary is a setup requirement for Claude peers, not advice.
